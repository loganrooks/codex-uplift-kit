...
**Disposition:** deferred.
**Target:** v0.3.
**Why not now:** v0.2 fixed local plugin path/mode correctness for the personal
default case, but full publishing/update/cache lifecycle and non-default
marketplace behavior need live Codex verification.
**Open seam:** plugin install mode separated from standalone skills; manifest
records plugin-owned files; marketplace generation is centralized.
...
### D-012A — Non-default plugin marketplace path probe

**Disposition:** platform-dependent.
**Target:** v0.3.
**Why not now:** v0.2 supports default personal marketplace-root semantics and
warns when a custom Codex home sits outside the user-home marketplace root. Live
Codex acceptance of absolute local `source.path` fallback should be probed
before the package documents that case as portable.
**Deferred-to artifact:** `.codex-uplift/v0.3-handoff.md`.
**Open seam:** marketplace path generation is centralized in the installer and
covered by tests for default and external-home behavior.
**Do-not-foreclose:** Do not scatter marketplace path assumptions or silently
write broken relative paths.
**Acceptance:** Codex client/plugin docs or a local restart/install probe confirms
absolute local paths, or the package switches to candidate-only metadata for
external homes.
**Trigger:** User installs plugin mode with custom `--home` outside user home, or
plugin docs clarify absolute local path behavior.
